...
void show_ports(avr_t *avr) {
  ...
  while (port) {
    printf("%s %s %08x %d\n", port->kind,
           port->irq_names ? *port->irq_names : NULL,
           port->irq_ioctl_get, port->irq_count);
    port = port->next;
  }
}

avr_cycle_count_t termination_timer(struct avr_t *avr,
...
int main(int argc, char *argv[]) {
  ...
  avr_irq_register_notify(avr_io_getirq(avr, AVR_IOCTL_UART_GETIRQ('0'),
                          UART_IRQ_OUTPUT), uart_output_cb, NULL);

  /*	VCD file initialization */
  printf("*** Initializing VCD Output ***\n");
  avr_vcd_init(avr, "wave.vcd", &vcd_file, 10000 /* usec */);
  avr_vcd_add_signal(&vcd_file, avr_io_getirq(avr, AVR_IOCTL_IOPORT_GETIRQ('B'), 5), 1, "B5" );
  avr_vcd_add_signal(&vcd_file, avr_io_getirq(avr, AVR_IOCTL_IOPORT_GETIRQ('B'), 4), 1, "B4" );

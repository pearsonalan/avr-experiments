...
HRESULT SimulatorInterface::connectSim(HWND hwnd) {
	...
	hr = buildDefinition();
	if (FAILED(hr)) {
		winfx::DebugOut(L"Error in buildDefinition: %08X", hr);
	...
	hr = defineEvents();
	if (FAILED(hr)) {
		winfx::DebugOut(L"Error in defineEvents: %08X", hr);
		SimConnect_Close(sim_);
		sim_ = INVALID_HANDLE_VALUE;
		return hr;
	}
...
HRESULT SimulatorInterface::defineEvents() {
	CHECK_OR_FAIL(SimConnect_MapClientEventToSimEvent(sim_, SimEventLandingLightsOn, "LANDING_LIGHTS_ON"));
	CHECK_OR_FAIL(SimConnect_MapClientEventToSimEvent(sim_, SimEventLandingLightsOff, "LANDING_LIGHTS_OFF"));
	CHECK_OR_FAIL(SimConnect_MapClientEventToSimEvent(sim_, SimEventToggleLandingLights, "LANDING_LIGHTS_TOGGLE"));
	CHECK_OR_FAIL(SimConnect_MapClientEventToSimEvent(sim_, SimEventTaxiLightsOn, "TOGGLE_TAXI_LIGHTS"));
...
void SimulatorInterface::sendEvent(SimulatorEvent event) {
	winfx::DebugOut(L"TransmitClientEvent %d\n", event);
	HRESULT hr = SimConnect_TransmitClientEvent(
		sim_, SIMCONNECT_OBJECT_ID_USER, static_cast<SIMCONNECT_CLIENT_EVENT_ID>(event),
		1, SIMCONNECT_GROUP_PRIORITY_HIGHEST, 
		SIMCONNECT_EVENT_FLAG_GROUPID_IS_PRIORITY);
	if (FAILED(hr)) {
		winfx::DebugOut(L"Failed to transmit client event: %08x", hr);
	}
}

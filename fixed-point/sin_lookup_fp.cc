...
void test(double d) {
    ...
    printf("  Lookup SIN = %0.9f\n", static_cast<double>(SinLookupWithInterpolation(f)));
    printf("  Actual = %0.9f\n", sin(ToRadians(d)));
    printf("  DELTA = %0.9f\n", sin(ToRadians(d)) - static_cast<double>(SinLookupWithInterpolation(f)));
    printf("\n");
}
